 ...
 * <p/>
 * This class is used to serialize and decode data in Base64 format
...
*/
     ...
     * @param iLen number of bytes to encode starting from the beginning (<= in.length)
     * @return character array w/ encoded bytes
     */
...
/*
    public static byte[] decode(final char[] in, final int fromIndex) {
        int iLen = in.length - fromIndex;
        if (iLen % 4 != 0) {
            throw new IllegalArgumentException("Length of Base64 encoded input string is not a multiple of 4.");
        }
        while (iLen > 0 && in[iLen - 1] == '=') {
        ...
        final int oLen = (iLen * 3) / 4;
        final byte[] out = new byte[oLen];
        for (int ip = fromIndex, op = 0; ip < iLen;) {
            final int i0 = in[ip++];
            final int i1 = in[ip++];
            final int i2 = ip < iLen ? in[ip++] : 'A';
            final int i3 = ip < iLen ? in[ip++] : 'A';
