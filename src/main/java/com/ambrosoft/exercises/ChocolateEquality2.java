 ...
 * This version 2 will reuse array
 */
public class ChocolateEquality2 {

    /*
        the original array will be reused
        we consider only elements that are not yet equalized, from start index to end of the array
     */

    static void equalize(final int[] counts) {
        ...
        // array is sorted, starts with one or more zeros

        int start = 0, counter = 0;
        int nonZeroIdx;
        while ((nonZeroIdx = findFirstNonZero(counts, start, end)) >= 0) {
            final int val = counts[nonZeroIdx];
            final int delta = val >= 5 ? 5 : val >= 2 ? 2 : 1;

            // first add to all (including leading zeros)
            for (int i = start; i < end; ++i) {
                counts[i] += delta;
            }
            // subtract back from item to be skipped
            counts[nonZeroIdx] -= delta;
            // it is evident that skipping a slot is equivalent to subtracting from it relative to all the others
            // array stays sorted after that and minimum is at counts[start] and is delta
            // that delta can now be subtracted from all values, but values to the left of nonZeroIdx can be ignored
            for (int i = start = nonZeroIdx; i < end; ++i) {
                counts[i] -= delta;
            }
            ++counter;
        }
    ...
    static int findFirstNonZero(final int[] counts, final int start, final int end) {
        for (int i = start; i < end; i++) {
            if (counts[i] > 0) {
                return i;
            }
