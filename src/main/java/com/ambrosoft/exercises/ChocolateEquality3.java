 ...
 * <p>
 * Final version will use observations
 * 1) skipping anyone while given N to all others is equivalent to subtracting N from the one skipped relative to others
 * 2) we need to reduce all surplusses independently
 * 3) each surplus will be reduced by 1s,2s, and 5s as in (shortest) coin change; can reuse MEMO table
 * 4) shortest way to equalize will be equal to MAX MIN "coin change"
 */
public class ChocolateEquality3 {
    // https://hackerranksolutionc.blogspot.com/2017/09/equal-hackerrank-solution.html

    // short and working solution found above
    static long find_min_actions(int[] cookies) {
        Arrays.sort(cookies);
        final int minElement = cookies[0];
        long sum = Long.MAX_VALUE;
        for (int base = 3; --base >= 0; ) {
            int current_sum = 0;
            for (int i = cookies.length; --i >= 0; ) {
                final int delta = cookies[i] - minElement + base;
    ...
    static long equalize(final int[] counts) {
        final int end = counts.length;

        final int[] coinChangeCounts;
        {
            ...
            // reduce to surpluses and find the biggest
            int biggest = -1;
            for (int i = end; --i >= 0; ) {
                if ((counts[i] -= smallest) > biggest) {
                    biggest = counts[i];
                }
            }
            coinChangeCounts = new int[biggest + 3];
            for (int i = biggest + 3; --i > 0; ) {
                coinChangeCounts[i] = i / 5 + i % 5 / 2 + i % 5 % 2;
            }
        }

        int minOpCount = Integer.MAX_VALUE;

        // key to solution is trying out 3 different 'offsets' to modify deltas to get better overall sum of min coin changes
        for (int base = 3; --base >= 0; ) { // 2,1,0
            int opCount = 0;
            for (int i = end; --i >= 0; ) {
                opCount += coinChangeCounts[counts[i] + base];
            }
            minOpCount = Math.min(minOpCount, opCount);
        }
        System.out.println("minOpCount = " + minOpCount);
        return minOpCount;
    }

    public static void main(String[] args) throws FileNotFoundException {
        equalize(new int[]{2, 2, 3, 7});
        equalize(new int[]{3, 7, 7, 7});
        equalize(new int[]{1, 5, 5, 10, 10});   // should be 7 steps?
        equalize(new int[]{1, 5, 5});   // should be 3 steps?
        if (false) {
            return;
        }
...
//
        final Scanner in = new Scanner(new FileInputStream("/Users/jacek/Downloads/input03.txt"));
